...
<%
	...
	DayOpenBean bean=null;
if(id!=null){
	try{
	int ad_open_days_id=Integer.parseInt(id);
	bean=new DayOpenDao().getDayOpenById(ad_open_days_id);
	 ...
	 <div class="portlet box purple">
	  <div class="portlet-title">
	   ...
	   <div class="portlet-body">
	    <form id="frmOpenDays" action="AdDayOpen?action=update&ad_open_days_id=<%=bean.getAd_open_days_id() %>" method="post" autocomplete="off">
			<table class="table table-bordered">
				<tr>
					<td width="15%">Change Date : <span class="red">*</span></td>
					<td width="35%"><input class="form-control input-medium date-picker" type="text" name="open_days" id="open_days" value="<%=new SimpleDateFormat("dd/MM/yyyy").format(bean.getOpen_days()) %>" /><label class="error"></label></td>
					<%-- <td width="15%">Status : <span class="red">*</span></td>
					<td width="35%">
					<select class="form-control input-medium" name="status" id="status">
						<option value="<% if(bean.getClosing_status()!=true){out.print("OPEN");}else{out.print("CLOSE");}%>"><%if(bean.getClosing_status()!=true){out.print("OPEN");}else{out.print("CLOSE");}%></option>
