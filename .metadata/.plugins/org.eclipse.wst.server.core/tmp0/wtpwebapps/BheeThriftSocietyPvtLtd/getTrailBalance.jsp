...
if(llist!=null){
	...
	for(LedgerAccountBean bean:llist){		
		...
		if(bean.getAd_ac_type_id()==1 || bean.getAd_ac_type_id()==2){
				...
				TransactionBean tb = new TransactionDao().getLedgerOpeningBal_2(bean.getAd_account_id(), fdate);
					 ...
					 bal=0;
						...
						if(tb.getCramt()!=0.00)
						{
							if(bean.getAd_ac_type_id()==1)
						     ...
						     total_opniing_bal+=bal;
					
				}
		}				
					
					TransactionBean tb1 = new TransactionDao().getLedgerBalBetweendates_2(bean.getAd_account_id(), fdate ,tdate);
					if(tb1!=null){
						
					
							
							 bal=0;
							/* if(tb1.getDramt()!=0.00)
							{
								if(bean.getAd_ac_type_id()==1)
									bal=bal+tb1.getDramt();
								else if(bean.getAd_ac_type_id()==2)
									bal=bal-tb1.getDramt();
								else if(bean.getAd_ac_type_id()==4)
									bal=bal+tb1.getDramt();
							...
							if(tb1.getCramt()!=0.00)
							{
								if(bean.getAd_ac_type_id()==1)
									bal=bal-tb1.getCramt();
								else if(bean.getAd_ac_type_id()==2)
									bal=bal+tb1.getCramt();
								else if(bean.getAd_ac_type_id()==4)
									bal=bal-tb1.getCramt();
